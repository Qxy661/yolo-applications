"""
钢珠数据 EDA（Exploratory Data Analysis）
...
def analyze():
    ...
    ax.set_title(f"每图钢珠数分布 (均值 {np.mean(n_balls):.1f})")
    ax.set_xlabel("钢珠数"); ax.set_ylabel("图数")
    ...
    ax.set_title(f"钢珠像素尺寸 (均值 {mean_w:.0f}x{mean_h:.0f})")
    ax.set_xlabel("宽(px)"); ax.set_ylabel("高(px)")
    ...
    ax.set_title("钢珠位置分布（画面归一化）")
    ax.set_xlabel("x"); ax.set_ylabel("y")

    # 4. 反光占比
    ax = axes[1, 1]
    ax.hist(highlight_ratio, bins=30, edgecolor="black")
    ax.set_title(f"bbox内高光占比分布 (均值 {np.mean(highlight_ratio):.2f})")
    ax.set_xlabel("高光占比"); ax.set_ylabel("频数")
    ...
    print(f"\n统计:")
    print(f"  每图钢珠数: 均值 {np.mean(n_balls):.1f}, 范围 {min(n_balls)}-{max(n_balls)}")
    print(f"  钢珠尺寸: 均值 {mean_w:.1f}x{mean_h:.1f}px (归一化 {float(sizes[:, 0].mean()):.3f})")
    print(f"  高光占比: 均值 {np.mean(highlight_ratio):.2f} (反光是普遍特征)")
